...
drush cr
curl -v -H "Content-Type:application/json" -X POST -d '{"source":"http://test_user:pass@localhost/relaxed/default","target":"localhost:5984/test_db"}' http://localhost:5984/_replicate | tee /tmp/test_couchdb.txt
